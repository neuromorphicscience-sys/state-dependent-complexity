...
def compute_metrics(pop_rate_hz, spike_counts, duration_s, bin_ms):
    """
    pop_rate_hz: [T] population mean firing rate sampled every bin_ms
    spike_counts: [N] total spikes per neuron after warmup
    """
    ...
    if np.any(valid) and np.sum(pxx[valid]) > 0:
        ...
        spectral_entropy = float(-np.sum(pnorm * np.log(pnorm + 1e-12)) / np.log(len(pnorm)))
    else:
    ...
    rate_cv_across_neurons = float(np.std(active) / (np.mean(active) + 1e-8)) if len(active) else 0.0

    # heuristic rhythm score: rewards narrowband rhythmic population activity,
    # while suppressing trivial silence and extreme instability.
